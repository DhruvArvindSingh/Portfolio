...
def generate_with_openrouter(prompt, model="moonshotai/kimi-k2"):
    """Generate code using OpenRouter API with Kimi K2"""
    ...
    headers = {
        ...
        'X-Title': 'GitHub Actions Code Generation'
    }
...
def generate_code_with_kimi(task_description, codebase_context):
    """Generate code using Kimi K2 via OpenRouter"""
    
    prompt = f"""
You are an expert software developer working on a Next.js TypeScript application.
...
- Follow Next.js patterns and best practices
...
def create_branch_and_commit(branch_name, generated_content):
    ...
    try:
        ...
        subprocess.run(['git', 'config', 'user.name', 'Kimi K2 Bot'], check=True)
        subprocess.run(['git', 'config', 'user.email', 'kimi-bot@example.com'], check=True)

        # Create and checkout new branch
        subprocess.run(['git', 'checkout', '-b', branch_name], check=True)
        
        # Create a summary file with the generated content
        with open('KIMI_GENERATED.md', 'w') as f:
            f.write(f"# Kimi K2 Generated Code\n\n")
        ...
        subprocess.run(['git', 'commit', '-m', f'Add Kimi K2 generated code\n\nTask: {os.environ["TASK_DESCRIPTION"]}\n\nGenerated by Kimi K2 via OpenRouter'], check=True)
...
def create_pull_request(branch_name, task_description, generated_content):
    ...
    pr_body = f"""## 🤖 Kimi K2 Generated Code

**Task Description:** {task_description}

This pull request contains code generated by Kimi K2 via OpenRouter based on the requested feature.
...
---
*This PR was created automatically by Kimi K2*
"""
    
    pr_data = {
        'title': f'🤖 Kimi K2 Generated: {task_description[:50]}{"..." if len(task_description) > 50 else ""}',
        'head': branch_name,
        'base': 'main',
        'body': pr_body
    }
...
def main():
    ...
    # Generate code with Kimi K2
    print("🧠 Generating code with Kimi K2...")
    generated_content = generate_code_with_kimi(task_description, codebase_context)
